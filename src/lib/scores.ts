...
export function compileScores(
  ...
  Object.entries(basePredictions ?? {}).forEach(([episodeNumber, predictionsMap]) => {
    ...
    const shauhinModeActive =
      ...
      && ((shauhinModeRules.startWeek === 'Custom'
        ...
        || (shauhinModeRules.startWeek === 'After Merge'
          && episodeNum > (keyEpisodes.mergeEpisode?.episodeNumber ?? Infinity))
    ...
    Object.values(refEvents).forEach(event => {
      ...
      event.references.forEach(reference => {
        ...
        if (reference.type === 'Tribe') {
          findTribeCastaways(tribesTimeline, eliminations, reference.id, episodeNum).forEach(
            castaway => {
              scores.Castaway[castaway] ??= [];
              scores.Castaway[castaway][episodeNum] ??= 0;
              scores.Castaway[castaway][episodeNum] += points;
