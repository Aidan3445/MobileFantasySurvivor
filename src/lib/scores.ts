...
export function compileScores(
  ...
  Object.entries(baseEvents).forEach(([episodeNumber, events]) => {
    ...
    Object.values(events).forEach(event => {
      ...
      if (
        !ScoringBaseEventNames.includes(event.eventName as ScoringBaseEventName)
      )
        return;
      eventCastaways.forEach(castaway => {
        ...
        const cmIndex = Math.min(
          episodeNum,
          (selectionTimelines.castawayMembers[castaway]?.length ?? 0) - 1
        );
        const leagueMember =
          selectionTimelines.castawayMembers[castaway]?.[cmIndex];
  ...
  Object.entries(customEvents.events).forEach(([episodeNumber, refEvents]) => {
    ...
    Object.values(refEvents).forEach(event => {
      ...
      const points = customEventRules.find(
        r => r.eventName === event.eventName
      )?.points;
      if (!points) return;
      ...
      event.references.forEach(reference => {
        ...
        if (reference.type === 'Castaway') {
          ...
          const leagueMember =
            selectionTimelines.castawayMembers[reference.id]?.[cmIndex];
          // if the castaway was not selected at this episode, don't score the member
          if (!leagueMember) return;
          scores.Member[leagueMember] ??= [];
          scores.Member[leagueMember][episodeNum] ??= 0;
  ...
  Object.entries(customEvents.predictions).forEach(
    ([episodeNumber, predictionsMap]) => {
      ...
      Object.values(predictionsMap)
        ...
        .forEach(prediction => {
          const points = customEventRules.find(
            r => r.eventName === prediction.eventName
          )?.points;
          if (!points) return;
  ...
  Object.entries(selectionTimelines.memberCastaways).forEach(
    ([memberId, castaways]) => {
      ...
      for (
        let episodeNumber = firstPickEpisode;
        episodeNumber < eliminations.length;
        episodeNumber++
      ) {
  ...
  const episodes =
    Math.max(...Object.values(scores.Castaway).map(s => s.length)) - 1;
