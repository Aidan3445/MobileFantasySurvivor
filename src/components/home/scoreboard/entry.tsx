...
export default function CastawayEntry({
  ...
  const fontScale = useWindowDimensions().fontScale.toFixed(2);

  return (
    <View key={castawayId} className={cn('flex-row gap-x-1 p-0.5', divideY(index))}>
      {!allZero && (
        <>
          <View
            className='w-11 items-center justify-center rounded px-1 h-6'
          ...
          <View
            className='w-10 items-center justify-center rounded px-1 h-6'
            style={{ backgroundColor: color }}>
            <Text
              className='text-center font-medium'
              maxFontSizeMultiplier={1.2}
      ...
      <View
        className='flex-row flex-1 items-center justify-center rounded px-2 h-6'
          ...
          <Popover.default
            from={
              <Pressable className='flex-row items-center'>
                <FlameKindling size={14 * +fontScale} color={getContrastingColor(colorOverride)} />
                <Text className='text-muted-foreground' maxFontSizeMultiplier={1}>
                  ({castaway.eliminatedEpisode})
                </Text>
              </Pressable>
            }>
